...
public final class LocalGameRepository extends GameRepository {
    ...
    private static final int MAX_REPO_INCREMENT = 6;
    ...
    public LocalGameRepository() {
        synchronized (LocalGameRepository.class) {
        	int increment = 0;
            while (theLocalRepoServer == null && increment <= MAX_REPO_INCREMENT) {
                try {
                	int portToTry = REPO_SERVER_PORT+increment;
                    theLocalRepoServer = HttpServer.create(new InetSocketAddress(portToTry), 0);
                    ...
                    theLocalRepoURL = "http://127.0.0.1:" + portToTry;
                } catch (IOException e) {
                	System.out.println("Failed to initialize local repo on port " + (REPO_SERVER_PORT+increment) + "...");
                	increment ++;
//                    throw new RuntimeException(e);
                }
            }
            if(increment > MAX_REPO_INCREMENT) {
            	System.out.println("ERROR: could not find a suitable port to initialize a local repo.");
            }
        }
