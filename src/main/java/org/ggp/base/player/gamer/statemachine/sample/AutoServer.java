...
public final class AutoServer extends JPanel implements ActionListener
{
    static void createAndShowGUI(AutoServer serverPanel, String title)
    {
    ...
    public static void main(String[] args)
    {
        ...
        final AutoServer serverPanel = new AutoServer();
        ...
        //To make sure the player list has been populated, wait a while
      ...
      //*************** Designate Games here ******************
//        gameSelector.getSelectedGameRepository().getGame(gameName);
//        serverPanel.gameSelector.getSelectedGameRepository()


        serverPanel.genGames("checkers", 22, 12, 100, false);
//        serverPanel.genGames("checkers", 60, 60, 1, false);
//        serverPanel.genGames("queens08ug", 20, 15, 1, false);
//        serverPanel.genGames("ticTacToe", 10, 5, 3, false);
    }
    ...
    public void genGames(String gameName, int startClock, int playClock, int repetitions, boolean shouldScramble) {
    	genGames(gameName, startClock, playClock, repetitions, shouldScramble, true, true, false, false);
    }

    public void genGames(String gameName, int startClock, int playClock, int repetitions, boolean shouldScramble, boolean shouldQueue, boolean shouldDetail, boolean shouldSave, boolean shouldPublish) {
    	//This bit selects the game called gameName
    	if(lastGameName == null || !gameName.equals(lastGameName)) {
	        theGame = gameSelector.getSelectedGameRepository().getGame(gameName);
	        lastGameName = gameName;

	        // a lot (all?) of the rest of this if statement deals with setting up UI elements for the selected game. This can probably be cut out.
	        ...
	        runButton.setEnabled(false);
	        if (theGame == null) {
	        	System.out.println("ERROR: game " + gameName + " not found.");
	            return;
	        }
        ...
        //This bit schedules the game
        synchronized (scheduler) {
            for (int i = 0; i < repetitions; i++) {
                scheduler.addPendingMatch(new PendingMatch("Base", theGame, new ArrayList<PlayerPresence>(thePlayers), -1, startClock, playClock, shouldScramble, shouldQueue, shouldDetail, shouldSave, shouldPublish));
